...
export default function AllDeals() {
    ...
    async function fetchAllDeals() {
        ...
        try {
            ...
            if (response.success) {
                ...
                const filteredDeals = deals.filter(product => product.priceAfterDiscount);
    ...
    const dealsWithDiscount = alldeals.filter(product => product.priceAfterDiscount);
                    ...
                    {Array.from({ length: totalPages }, (_, index) => (
                        <button
                            ...
                            className={`px-3 py-1 rounded ${
                                page === index + 1 ? 'bg-primary-500 text-white' : 'bg-gray-200'
                            }`}
                        >
                            {index + 1}
                        </button>
